...
public class PngManager {
    ...
    public static Tile[] loadPngFile(String filepath) throws IOException{
        ...
        try{
            ...
            if(path.toFile().exists()){
             ...
             System.err.println("com.sfc.sf2.spellAnimation.io.PngManager.importPng() - Error while parsing PNG data : "+e);
             throw e;
        }                
        return tiles;                
    }
    ...
    private static Color[] buildColors(IndexColorModel icm){
        ...
        if(icm.getMapSize()>16){
            System.out.println("com.sfc.sf2.spellAnimation.io.PngManager.buildColors() - PNG FORMAT HAS MORE THAN 16 INDEXED COLORS : "+icm.getMapSize());
        }
    ...
    public static void exportPng(SpellAnimation spellAnimation, String filepath, int selectedPalette){
        try {
            System.out.println("com.sfc.sf2.spellAnimation.io.PngManager.exportPng() - Exporting PNG files and palettes ...");
/*            Tile[][] frames = spellAnimation.getFrames();
            for(int i=0;i<frames.length;i++){
                String framePath = filepath + "-frame-" + String.valueOf(i) + ".png";
                writePngFile(frames[i],spellAnimation.getType(),framePath); 
            }
            Color[][] palettes = spellAnimation.getPalettes();
...
*/                           
            System.out.println("com.sfc.sf2.spellAnimation.io.PngManager.exportPng() - PNG files and palettes exported.");
        } catch (Exception ex) {
            Logger.getLogger(PngManager.class.getName()).log(Level.SEVERE, null, ex);
        }
    }    
    
    public static void writePngFile(Tile[] tiles, int spellAnimationType, String filepath){
        try {
/*            System.out.println("com.sfc.sf2.spellAnimation.io.PngManager.exportPng() - Exporting PNG file ...");
            SpellAnimationLayout spellAnimationLayout = new SpellAnimationLayout();
            BufferedImage image = spellAnimationLayout.buildImage(true);
